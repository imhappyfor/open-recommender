...
class RecommendationItem:
    ...
    site_id: str
    site_name: str
    score: float  # 0.0-1.0 or 0-10, normalized by site
    reason: str | None = None
    metadata: dict[str, Any] = field(default_factory=dict)  # title, description, thumbnail, etc.
    timestamp: str | None = None  # when the site created this recommendation
...
class AggregatedRecommendation:
    ...
    item_id: str
    consensus_score: float  # 0-1, based on number of sites recommending
    freshness_boost: float  # 0-1, based on recency
    affinity_boost: float  # 0-1, based on user preference match
    final_score: float  # weighted combination of above
    sources: list[RecommendationItem]  # all sources recommending this item
    best_metadata: dict[str, Any]  # metadata from highest-scoring source
...
class AggregatedFeed:
    ...
    def __init__(self, profile: "ORFProfile"):
    ...
    def _compute_consensus_score(self, sources: list[RecommendationItem], total_sites: int) -> float:
        """Consensus: normalized by number of unique sites recommending."""
        if not sources or total_sites == 0:
            return 0.0
        unique_sites = len({s.site_id for s in sources})
    ...
    def _compute_freshness_boost(self, items: list[RecommendationItem]) -> float:
        ...
        if not items or not items[0].timestamp:
            return 0.5  # neutral boost for items with no timestamp
    ...
    def _compute_affinity_boost(self, item_id: str) -> float:
        ...
        # Simple heuristic: high-weight topics (>0.5) contribute to affinity
        high_weight_topics = {
            t for t, pref in self.profile.topics.items() if pref.weight > 0.5
        }
        if not high_weight_topics:
            return 0.5

        # Check if any topic segments appear in item_id
        ...
        for topic in high_weight_topics:
            ...
            if topic.lower() in item_lower:
                ...
                # Check individual segments (namespace:path/subpath)
                segments = topic.replace(":", "/").split("/")
                for segment in segments:
                    if segment and segment in item_lower:
                        matches += 1
                        break  # Count only once per topic

        return min(1.0, 0.5 + (matches / len(high_weight_topics)) * 0.5)

    def aggregate(
        self, *, consensus_weight: float = 0.4, freshness_weight: float = 0.3,
        ...
        Returns:
            List of AggregatedRecommendation sorted by final_score descending.
        """
        aggregated = []
        total_sites = len({s.site_id for sources in self._recommendations.values() for s in sources})
        ...
        for item_id, sources in self._recommendations.items():
            ...
            # Best metadata from highest-scoring source
            best_source = max(sources, key=lambda s: s.score)
    ...
    def top_n(
        self, n: int = 20, *, consensus_weight: float = 0.4,
        freshness_weight: float = 0.3, affinity_weight: float = 0.3
    ) -> list[AggregatedRecommendation]:
        """Return top N aggregated recommendations."""
        all_recs = self.aggregate(
